...
void block_sparsematrix_mm_load_MPI(struct block_sparsematrix_t * M, char const * filename)
{
        ...
        for (long u = 0; u < mySize; u++) {
                ...
                if (MPIrank == MPIsize -1 && (u & 0xffff) == 0xffff) {
                        ...
                        printf("\r  - Reading %s: %.1f%% (%.1f MB/s) pivots = %d", matrix_filename, percent, rate, pivots);
                }
        }
...
void unique_block_vector_product(u32 *y, struct unique_block_t const * M, u32 const * x, bool transpose)
{
        ...
        int startI = MPIi * calcBlockSide(M->nrows, M->nblocksSqrt);
        int startJ = MPIj * calcBlockSide(M->ncols, M->nblocksSqrt);
        for (long i = 0; i < calcBlockSide(nrows,M->nblocksSqrt) * n; i++)
                y[i] = 0;
...
void verbosity()
{
        ...
        printf("\r    - iteration %d / %d. %.3fs per iteration. ETA: %s, pivots = %d", 
                n_iterations, expected_iterations, per_iteration, ETA, pivots);
        fflush(stdout);
}
...
u32 * unique_block_lanczos(struct unique_block_t const * M, int n, bool transpose)
{
        ...
        while (true) {
                ...
                u32 winv[n * n];
                u32 d[n];
                pivots = semi_inverse(vtAv, winv, d);
                stop = (pivots == 0);
...
u32 * block_lanczos(struct sparsematrix_t const * M, int n, bool transpose)
{
        ...
        for (long i = 0; i < block_size; i++)
                v[i] = i % prime;
        // save_vector_block("startVec.mtx",
        // nrows, n, v);
        ...
        while (true) {
                ...
                // if (it <= 1){
                //         sprintf(name, "tmp%d.mtx", it);
                //         save_vector_block(name, ncols, n, tmp);
                // }
...
void singleProc(int argc, char **argv){
        ...
        if (kernel_filename){
                char kernel_filenameS[100];
                sprintf(kernel_filenameS, "single_proc_%s",kernel_filename);
                save_vector_block(kernel_filenameS, right_kernel ? M.ncols : M.nrows, n, kernel);
                }
        else
